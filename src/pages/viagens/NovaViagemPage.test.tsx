...
test("A04: tocar o fornecedor vazio mostra o erro no campo, mas o aviso de atenção só aparece ao salvar", async () => {
  montar();
  await screen.findByRole("heading", { name: /Nova viagem/ });
  fireEvent.keyDown(window, { key: "Enter", ctrlKey: true });
  const reserva1 = await screen.findByRole("region", { name: /Reserva 1/ });

  fireEvent.blur(within(reserva1).getByLabelText("Fornecedor"));

  expect(await screen.findByText("Escolha o fornecedor")).toBeInTheDocument();
  expect(screen.queryByRole("button", { name: /precisa(m)? de atenção/ })).toBeNull();

  fireEvent.click(screen.getByRole("button", { name: "Salvar viagem" }));

  expect(await screen.findByRole("button", { name: /precisa(m)? de atenção/ })).toBeInTheDocument();
});
